# adapted from /home/mmordig/reinforcement/text-generation-inference/server/text_generation_server/utils/logits_process.py
# mostly adapted imports
...
class GrammarType:
    GRAMMAR_TYPE_JSON = 0
    GRAMMAR_TYPE_REGEX = 1

# added
class StatefulGrammarLogitProcessor:
    """Keeps track of the state per sequence in the batch"""
    def __init__(self, grammar_logits_processor: "GrammarLogitProcessor", fsm_states: Optional[List[int]] = None):
        self.grammar_logits_processor = grammar_logits_processor
        self.fsm_states = fsm_states
        self.cur_seq_len = None # current length of sequences in the batch (finished seqs are dropped/can be ignored)
    ...
    def __call__(
        ...
        if self.fsm_states is None:
            self.fsm_states = [0] * input_ids.shape[0]
        
        if self.cur_seq_len is None:
            self.cur_seq_len = input_ids.shape[1]
            
        # update fsm states with last token if necessary (once beam search advances to next token)
        seq_len = input_ids.shape[1]
        if seq_len > self.cur_seq_len:
            assert seq_len == self.cur_seq_len + 1
            self.fsm_states = [
                self.grammar_logits_processor.advance(input_ids[i, -1], self.fsm_states[i])
                for i in range(input_ids.shape[0])
            ]
            self.cur_seq_len = seq_len
            
        # todo: optimization: beam replicates the same sequence several times
        # HeterogeneousGrammarLogitProcessor
        
        return torch.concat([
            self.grammar_logits_processor(scores[[i]], self.fsm_states[i]) 
...
class GrammarLogitProcessor(LogitsProcessor):
    ...
    def _cached_compile_fsm(grammar_type, schema, tokenizer):
        ...
        # fsm = RegexFSM(schema, tokenizer) # todo
