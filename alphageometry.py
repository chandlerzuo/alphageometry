...
_MODE = flags.DEFINE_string("mode", "ddar", "either `ddar` (DD+AR) or `alphageometry`")
...
_VOCAB_PATH = flags.DEFINE_string("vocab_path", "", "path to the LM vocab file.")
...
def natural_language_statement(
    logical_statement: pr.Dependency, return_nl_also: bool
) -> str:
...
def proof_step_string(
    ...
    if return_nl_also:
        premises_nl = " & ".join(
            [
                natural_language_statement(p, return_nl_also)
                + " [{:02}]".format(refs[p.hashed()])
                for p in premises
            ]
        )
    ...
    premises_fl = " & ".join(
        [
            natural_language_statement(p, return_nl_also=False)
            + " [{:02}]".format(refs[p.hashed()])
            for p in premises
        ]
    )
    ...
    conclusion_nl = ""
    if return_nl_also:
        conclusion_nl = natural_language_statement(
            conclusion, return_nl_also=return_nl_also
        )
        if not last_step:
...
def write_solution(
    g: gh.Graph, p: pr.Problem, out_file: str, goal=None, return_nl_also=True
) -> Optional[str]:
    ...
    setup, aux, proof_steps, refs = ddar.get_proof_steps(g, goal, merge_trivials=False)
    ...
    for premises, [points] in setup:
        points_nl += " ".join([p.name.upper() for p in points]) + " "
        if not premises:
            continue
        premises_nl += [
            natural_language_statement(p, return_nl_also)
    ...
    solution_nl = f"Problem. Consider a graph containing points {points_nl.strip()} that satisfies the following conditions: \n"    
    solution_nl += ";\n".join(premises_nl)
    solution_nl += "."
    solution = copy.deepcopy(solution_nl)
    
    conclusion = proof_steps[-1][1][0]
    conclusion_nl = natural_language_statement(conclusion, return_nl_also=True)
    solution_nl += "\nProve that " + conclusion_nl + "."
    ...
    for premises, [points] in aux:
        aux_points_nl += " ".join([p.name.upper() for p in points]) + " "
        if return_nl_also:
            aux_premises_nl += [
                natural_language_statement(p, return_nl_also)
                + " [{:02}]".format(refs[p.hashed()])
    ...
    if len(aux) > 0:
        solution_nl += f"\nAugment the graph with points {aux_points_nl} that satisfy the following additional conditions: \n"
    ...
    solution += "\n\n * Proof steps:\n"
    prefix_cause = [
        "From", "Since", "Given", "Because", "As"
    ]
    prefix_conclusion = ["we can see that", "we can obtain that", "it follows that"]
    prefix_rule = ["according to", "by the rule of"]
    for i, step in enumerate(proof_steps):
        _, [con] = step
        fl, nl = proof_step_string(
            step,
            refs,
            last_step=i == len(proof_steps) - 1,
            return_nl_also=return_nl_also,
        )
        rule_name = r2name.get(con.rule_name, "")
        nl = nl.replace("\u21d2", f"{rule_name}\u21d2 ")
        this_premises, this_conclusion = nl.split("\u21d2")
        solution_nl += random.choice(prefix_cause) + " " + this_premises.strip() + ", "
        if con.rule_name in r2name:
            solution_nl += random.choice(prefix_rule) + " " + rule_name + ", "
        solution_nl += random.choice(prefix_conclusion) + " " + this_conclusion.strip() + ".\n"
